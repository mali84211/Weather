...
async function searchByCoordinates(latitude, longitude) {
  let weatherApi = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=15fb3b0b9dcf42cea83142040241001&q=${latitude},${longitude}&days=3`
  );
...
async function searchCity(cityName) {
  let weatherApi = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=15fb3b0b9dcf42cea83142040241001&q=${cityName}&days=3`
  );
  let apiResponse = await weatherApi.json();
  ...
  weatherData += `
         <div class="col" >
            <div class="card bg-success text-white ">
              <div class="card-header bg-success-subtle text-dark d-flex justify-content-between">
